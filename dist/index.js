...
class ReactNativeUpgradeHelperMCP {
    ...
    setupTools() {
        ...
        this.server.registerTool('get_upgrade_info', {
        ...
        this.server.registerTool('get_file_diff', {
    ...
    async getUpgradeInfo(fromVersion, toVersion, packageName = 'com.example.app', projectName = 'ExampleApp') {
    ...
    async getFileDiff(fromVersion, toVersion, fileName, packageName = 'com.example.app', projectName = 'ExampleApp') {
        const url = `https://react-native-community.github.io/upgrade-helper/`;
        const browser = await chromium.launch();
        const page = await browser.newPage();
        try {
            ...
            const fileDiff = await page.evaluate((targetFileName) => {
                const possibleSelectors = [
                    '[data-testid="file-diff"]',
                    '.file-diff',
                    '.diff-container',
